...
public class Program
{
    ...
    private static async Task StartServer(string[] args)
    {
        ...
        Log.Info($"Starting NitroxServer {NitroxEnvironment.ReleasePhase} v{NitroxEnvironment.Version} for Subnautica: Below Zero");
        ...
        try
        {
            ...
            else
            {
                ...
                {
                    string gameDir = NitroxUser.GamePath_BZ;
                    Log.Info($"Using game files from: {gameDir}");
                    return gameDir;
                });
            }

            NitroxServiceLocator.InitializeDependencyContainer(new BelowZeroServerAutoFacRegistrar());
