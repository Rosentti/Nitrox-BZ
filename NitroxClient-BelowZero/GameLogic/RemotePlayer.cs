...
public class RemotePlayer : INitroxPlayer
{
    ...
    public LifepodDrop LifepodDrop { get; private set; }
    ...
    public void SetLifepodDrop(LifepodDrop newLifepodDrop)
    {
        if (LifepodDrop != newLifepodDrop)
        {
            if (newLifepodDrop)
            {
                Attach(newLifepodDrop.transform, true);
            }
            else
            {
                Detach();
    ...
    private void SetupPlayerSounds()
    {
        ...
        static void CopyEmitter(FMODUnity.StudioEventEmitter src, FMODUnity.StudioEventEmitter dst)
        {
            dst.AllowFadeout = src.AllowFadeout;
            dst.CollisionTag = src.CollisionTag;
            dst.Event = src.Event;
        ...
        static void CopyCustomEmitter(FMOD_CustomEmitter src, FMOD_CustomEmitter dst)
        {
            dst.asset = src.asset;
        ...
        FMODUnity.StudioEventEmitter bubblesCustomEmitter = remotePlayerSoundsRoot.AddComponent<FMODUnity.StudioEventEmitter>();
        CopyEmitter(localPlayerBubbles.bubbleSound, bubblesCustomEmitter);

        if (fmodWhitelist.IsWhitelisted(bubblesCustomEmitter.Event, out float bubblesSoundRadius))
        {
            emitterController.AddEmitter(bubblesCustomEmitter.Event, bubblesCustomEmitter, bubblesSoundRadius);
        }
        else
        {
            Log.Error($"[{nameof(RemotePlayer)}] Manual created FMOD emitter for {nameof(PlayerBreathBubbles)} but linked sound is not whitelisted: ({bubblesCustomEmitter.Event})");
        }
        ...
        CopyCustomEmitter(waterAmbience.diveStartSplash, diveStartCustomEmitter);
