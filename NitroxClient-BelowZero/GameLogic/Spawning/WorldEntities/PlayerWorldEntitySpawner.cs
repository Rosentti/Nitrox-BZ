...
public class PlayerWorldEntitySpawner : IWorldEntitySpawner
{
    ...
    private void AttachToParent(RemotePlayer remotePlayer, GameObject parent)
    {
        ...
        else if (parent.TryGetComponent(out LifepodDrop escapePod))
        {
            Log.Debug($"Found LifepodDrop for {remotePlayer.PlayerName}.");
            remotePlayer.SetLifepodDrop(escapePod);
        }
        else
        {
            Log.Error($"Found neither SubRoot component nor LifepodDrop on {parent.name} for {remotePlayer.PlayerName}.");
        }
    }
    ...
    private bool IsSwimming(Vector3 playerPosition, Optional<GameObject> parent)
    {
        if (parent.HasValue)
        {
            ...
            Log.Debug($"Trying to find lifepod for {parent}.");
            parent.Value.TryGetComponent<LifepodDrop>(out LifepodDrop escapePod);
            if (escapePod)
            {
                Log.Debug("Found lifepod for player. Will add him and update animation.");
                return false;
            }
        }
