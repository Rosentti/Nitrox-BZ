...
public class FMODEmitterController : MonoBehaviour
{
    ...
    private readonly Dictionary<string, FMODUnity.StudioEventEmitter> studioEmitters = new();
    private readonly Dictionary<string, FMOD_StudioEventEmitter> oldStudioEmitters = new();
        ...
        foreach (MonoBehaviour behaviour in gameObject.GetComponentsInChildren<MonoBehaviour>(true))
        {
            switch (behaviour)
            {
                case FMOD_CustomEmitter customEmitter when this.Resolve<FMODWhitelist>().IsWhitelisted(customEmitter.asset.path, out float maxDistance):
                    ...
                    break;
                case FMODUnity.StudioEventEmitter studioEmitter when this.Resolve<FMODWhitelist>().IsWhitelisted(studioEmitter.Event, out float maxDistance):
                    AddEmitter(studioEmitter.Event, studioEmitter, maxDistance);
                    break;
            }
        }
    }
    ...
    public void AddEmitter(string path, FMODUnity.StudioEventEmitter studioEmitter, float maxDistance)
    {
        if (!customEmitters.ContainsKey(path))
        {
    ...
    public void AddEmitter(string path, FMOD_StudioEventEmitter studioEmitter, float maxDistance)
    {
        if (!customEmitters.ContainsKey(path))
        {
            oldStudioEmitters.Add(path, studioEmitter);
        }
    }
    ...
    public void StopStudioEmitter(string path, bool allowFadeout) {
        var emitter = studioEmitters[path].AliveOrNull();
        if (emitter != null) {
        ...
        var oldEmitter = oldStudioEmitters[path].AliveOrNull();
